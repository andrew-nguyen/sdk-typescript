import { grpcStatus, type StatusObject } from './grpc-status';
...
export function isRetryableError(status: StatusObject): boolean {
  ...
  if (status.code === grpcStatus.INTERNAL) {
 ...
 */
const retryableCodes = new Set<grpcStatus>([
  grpcStatus.UNKNOWN,
 ...
 */
function defaultInitialIntervalMs({ code }: StatusObject): number {
  // Backoff more on RESOURCE_EXHAUSTED.
  if (code === grpcStatus.RESOURCE_EXHAUSTED) {
    return 1000;
  }
  return 100;
}
